...
import { reactive, shallowRef, watch, defineComponent, mergeProps, unref, withCtx, createVNode, openBlock, createBlock, createCommentVNode, useSSRContext, ref, computed } from "vue";
import "hookable";
import { k as defineStore, l as useRuntimeConfig, m as useState, a as useNuxtApp, n as useMagicKeys, w as watchThrottled, o as unrefElement, t as toValue } from "../server.mjs";
import { toast } from "vue-sonner";
import * as localForage from "localforage";
import { useForwardPropsEmits, EditableSubmitTrigger, EditableCancelTrigger, EditableRoot, EditableArea, EditablePreview, EditableInput } from "radix-vue";
import { _ as _sfc_main$3, c as cn } from "./Header-CVL2gF3u.js";
...
const useDataStore = defineStore("data", () => {
  ...
  const setData = (key, value) => {
    data[key] = value;
    if (key === "css") dynamicCssService.injectCssEditor(value);
  };
...
var _processDefToken = (state, startLine, endLine, pos, label) => {
  var _a, _b;
  (_a = state.env).crossRef ?? (_a.crossRef = {});
  (_b = state.env.crossRef).labelToId ?? (_b.labelToId = {});
...
var processRef = (state, silent) => {
  var _a, _b;
  if (!((_a = state.env.crossRef) == null ? void 0 : _a.labelToId)) return false;
  ...
  if (!silent) {
    (_b = state.env.crossRef).list ?? (_b.list = []);
...
var postProcessDef = (state) => {
  ...
  for (const token of state.tokens) {
    if (token.type === "renderDefOpen") {
      token.meta = {
        ...token.meta,
        id: ((_b = (_a = state.env.crossRef) == null ? void 0 : _a.labelToId) == null ? void 0 : _b[token.meta.label]) ?? -1
      };
    }
  }
};
...
//# sourceMappingURL=Skeleton-DmysgnrS.js.map
